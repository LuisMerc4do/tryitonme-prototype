...
export async function POST(request: NextRequest) {
  try {
    ...
    const absoluteProductUrl = productImageUrl.startsWith('http')
      ? productImageUrl
      : new URL(productImageUrl, request.url).toString();

    const productImageResponse = await fetch(absoluteProductUrl);
...
- Result should look indistinguishable from a real photograph`;
    ...
    if (!generatedImageBase64) {
      ...
      throw new Error(`No image generated by AI. Details: ${JSON.stringify(errorDetails)}`);
    }
